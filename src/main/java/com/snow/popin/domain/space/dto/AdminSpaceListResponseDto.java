...
@Getter
@Builder
public class AdminSpaceListResponseDto {
    private Long id;
    ...
    public static AdminSpaceListResponseDto from(Space space) {
        // Venue에서 주소 정보 조합
        String fullAddress = buildFullAddress(space);

        return AdminSpaceListResponseDto.builder()
                .id(space.getId())
    ...
    private static String buildFullAddress(Space space) {
        if (space.getVenue() == null) {
            return space.getAddress() != null ? space.getAddress() : "주소 정보 없음";
        }
